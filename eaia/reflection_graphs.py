...
from eaia.llm_factory import get_llm, get_reasoning_llm
...
async def update_general(state: ReflectionState, config, store: BaseStore):
    reflection_model = get_reasoning_llm(disable_streaming=True)
...
async def determine_what_to_update(state: MultiMemoryInput):
    reflection_model = get_llm(disable_streaming=True)
